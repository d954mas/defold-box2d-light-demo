...
function Camera:update(dt)
    ...
    self.lpos = self.lpos - self.shake
    ...
    if self.following then
        ...
        self.follow_pos = self:follow_lerp_func(self.data.lpos, self.follow_pos, dt)
...
function Camera:recalculate_view_proj()
    ...
    local w_pos = self.wpos + self.lpos

    -- Absolute/world near and far positions for screen-to-world transform
    self.abs_near_z = w_pos.z - self.near_z
    self.abs_far_z = w_pos.z - self.far_z

    self.view = vmath.matrix4_look_at(w_pos, w_pos + self.wforward_vec, self.wup_vec)
    ...
    if (self.orthographic) then
        ...
        self.orthographic_border = {
            left_top = self:screen_to_world_2d(0, COMMON.RENDER.screen_size.h),
            right_bottom = self:screen_to_world_2d(COMMON.RENDER.screen_size.w, 0)
        }
    else
